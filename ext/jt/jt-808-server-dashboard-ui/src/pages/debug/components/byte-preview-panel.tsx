...
const ByteBitsPanel = ({ values }: { values: number[] }) => {
  if (!values.length) {
    return null;
  }
  ...
  if (values.length > 4) {
    return (
      <div className="rounded-lg border border-dashed border-border/70 bg-background px-2.5 py-2 text-xs text-muted">
        选择 1-4 字节查看 bit 位图
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-border/60 bg-background px-2.5 py-2">
      <div className="flex items-center justify-between gap-2">
        <span className="text-[11px] font-medium text-muted">Bits</span>
  ...
  return (
    <aside className="rounded-xl border border-border/60 bg-background-secondary/45 p-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-sm font-semibold">字节预览</div>
