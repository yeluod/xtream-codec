...
在 `quick-start/custom-annotation-server/` 下新增 Gradle module，并注册到项目根 `settings.gradle.kts`。
...
`XtreamDemoServerApp.java` 包含 `main` 方法，启动 TCP 服务器：
...
一个 `@DemoMessageHandler` 类中至少包含 3 个 Handler 方法：
...
文档位于 `docs/src/guide/server/samples/custom-demo-protocol/`，结构参考 `custom-protocol-sample-01`：
