...
类级注解，SHALL 标记某个类为 X-IoT Demo 协议的请求处理器。该注解须使用 `@XtreamRequestHandler` 作为元注解，以便被框架的扫描逻辑识别。
...
方法级注解，SHALL 标记该方法能处理的消息类型。使用 `@XtreamRequestHandlerMapping` 作为元注解，并通过 `msgType` 属性指定能处理的消息类型值。
...
分发逻辑 SHALL 继承 `AbstractSimpleXtreamRequestMappingHandlerMapping`，按报文中的 msgType 分发到对应处理方法。
...
服务端 SHALL 使用 `LengthFieldBasedFrameDecoder` 拆包，配置为：
- `maxFrameLength` = 1024
