...
提供共享的扩展元数据解析工具，统一 Key、ValueLength、ValueMatcher、字符集和 Map 元数据缓存逻辑，避免 TLV、Pair、Map 等编解码路径重复维护等价实现。
...
- **WHEN** `valueType.isPlaceholder()` 为 false 且 `codecCharset` 为 `UTF_8`/`ASCII`/`GBK`/`GB_2312`/`BCD_8421`/`HEX`
...
`SimpleMapMetadataRegistry` SHALL 使用实例方法和实例 `ConcurrentHashMap` 缓存 Map 元数据。
...
### Requirement: DefaultExtendMetaRegistry 委托 MetadataUtils

`DefaultExtendMetaRegistry` SHALL 将与 `MetadataUtils` 重复的 Key、ValueLength、ValueMatcher 和字符集解析逻辑委托给 `MetadataUtils` 完成。
